...
class Chemostat(bioreactor.Bioreactor):
    ...
    def check_pump_settings(self):
        ...
        pump_setting_parameters = zip(
            self.vials, bolus_volumes, dilutions_per_unit_time
        )
    ...
    def update(
        ...
        pump_params = zip(
            self._chemostat_settings.bolus,
            self._data_fits,
            self._chemostat_settings.rates,
            self._chemostat_settings.pump_ratios,
        )
        ...
        for vial, pump_commands in pump_settings.items():
            ...
            self.logger.info(f"chemostat initiated for vial {vial}", f"period {period}")
