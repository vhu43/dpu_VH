  ...
  if reactor_type.suffix == ".py":
    if reactor_type.stem not in ("bioreactor", "template", "__init__"):
      ...
      REACTOR_SETTINGS_FIELDS[reactor_type.stem]["tuple_fields"] = tuple([
        f.name for f in dataclasses.fields(setting_class) if
        isinstance(f.type, types.GenericAlias) and f.type.__origin__ is tuple
        and f.name != "base_settings"])

      REACTOR_SETTINGS_FIELDS[reactor_type.stem]["fields"] = tuple([
        f.name for f in dataclasses.fields(setting_class)
...
BASE_FIELDS = tuple(
  [f.name for f in dataclasses.fields(bioreactor.ReactorSettings)])
