...
class ReactorSettings:
  ...
  # Vial control
  vials: tuple[int, ...] = dataclasses.field(default_factory=int)
  temp: tuple[float, ...] = dataclasses.field(default_factory=tuple)
  stir: tuple[int, ...] = dataclasses.field(default_factory=int)
...
class Bioreactor(ABC):
  ...
  def __init__(self, name: str,
    ...
    self._logger = logging.LoggerAdapter(logging.getLogger(manager),
                                         {"name": self._name})
  ...
  def _format_cal(self, calibrations: type_hints.Calibration):
    ...
    for location, fit_data in to_process:
      ...
      curve_module = importlib.import_module("evolver_manager.fits."
                                             f"{module_name}")
  ...
  def _manage_temperature(self, temp_readings: tuple[float, ...],
                          config: type_hints.ExperimentalParams):
    ...
    if adjusting:
      self.logger.info("updating temperatures, local setpoint differs from"
                       " evolver setpoint")
      self._evolver.update_temperature(vials_to_adjust, new_temp_setpoints)
    else: #There is no config discrepency. Record this fact.
      self.logger.debug("actual temperature does not match configuration "
  ...
  def process_data(self, data: type_hints.DataDict
    ...
    for vial, od_fit, temp_fit, *_ in self._data_fits:
      ...
      try:
        ...
        self.logger.error(f"temperature read error for vial {vial},"
                          " setting to NaN")
        temp_reading = np.nan
  ...
  def pre_update(self, broadcast: type_hints.Broadcast, curr_time: float
  ...
  @classmethod
  def parse_fluid_usage(cls, update_msg
