...
from evolver_manager import bioreactors, evolver_controls
...
def check_settings(
    ...
    if len(settings) != num_fields:
        raise ValueError(
            f"{reactor} configuration missing a field. Expected "
...
def encode_start(name: str, working_directory: Path, config: dict[str, Any]):
    ...
    for reactor, vial_subset in reactors.items():
        settings[reactor] = check_settings(
            ...
            bioreactors.BASE_TUPLE_FIELDS,
            {"mem_len": "OD_values_to_average"},
        )
...
def build_reactor(
    ...
    base_settings = dict_to_dataclass(
