...
class EvolverManager(socketio.ClientNamespace):
  ...
  def __init__(self, namespace: str, master_dir: Path,
    ...
    self._active_reactors: dict[
      str, list[tuple[bioreactor.Bioreactor, Path]]] = {}
        ...
        for data_type in ["od_90", "od_135", "temperature"]:
          if raw_data[data_type]:
            for vial, od in zip(reactor.vials, reactor.od_readings):
              ...
              file_name = Path(working_dir, raw_data, data_type, str(vial))
              with open(file_name, "a", encoding="utf8") as f:
